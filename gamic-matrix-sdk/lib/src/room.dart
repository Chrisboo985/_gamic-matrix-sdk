...
class Room {
    ...
    if (membership == Membership.leave) {
      ...
      if (invitation != null && invitation.unsigned?['prev_sender'] != null) {
        final name = unsafeGetUserFromMemoryOrFallback(
                invitation.unsigned?.stringFor('prev_sender') ?? '')
            .calcDisplayname(i18n: i18n);
  ...
  Future<void> join({bool leaveIfNotFound = true}) async {
    try {
      ...
      final invitation = getState(EventTypes.RoomMember, client.userID!);
      if (invitation != null &&
          invitation.content.boolFor('is_direct') == true) {
  ...
  Future<void> removeFromDirectChat() async {
    ...
    for (final k in directChats.keys) {
      if (directChats[k] is List && directChats.listFor(k)?.contains(id) == true) {
        directChats.tryListFor(k)?.remove(id);
      }
    }
  ...
  Future<void> enableGroupCalls() async {
    ...
    if (currentPowerLevelsMap != null) {
      ...
      final eventsMap = newPowerLevelMap.mapFor('events') ?? {};
      eventsMap.addAll({

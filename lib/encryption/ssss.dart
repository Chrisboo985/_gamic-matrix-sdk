...
class SSSS {
  ...
  Future<String?> getCached(String type) async {
    ...
    bool isValid(dbEntry) =>
        ...
        client.accountData[type]?.content.tryMapFor('encrypted')?[dbEntry.keyId]
                ['ciphertext'] ==
            dbEntry.ciphertext;
  ...
  Future<String> getStored(String type, String keyId, Uint8List key) async {
    ...
    if (secretInfo.content.tryMapFor('encrypted')?[keyId] is! Map) {
      throw Exception('Wrong / unknown key');
    }
    final enc = secretInfo.content.tryMapFor('encrypted')?[keyId];
  ...
  Future<void> validateAndStripOtherKeys(
    ...
    final otherKeys =
        Set<String>.from((content.tryMapFor('encrypted') ?? {}).keys.where((k) => k != keyId));
    content.tryMapFor('encrypted')?.removeWhere((k, v) => otherKeys.contains(k));
    // yes, we are paranoid...
    if (await getStored(type, keyId, key) != secret) {
      throw Exception('Secrets do not match up!');
    }
    ...
    if (cacheTypes.contains(type)) {
      ...
      await client.database?.storeSSSSCache(
          type, keyId, content.tryMapFor('encrypted')?.tryMapFor(keyId)?.stringFor('ciphertext') ?? '', secret);
      onSecretStored.add(keyId);
    }
  }
  ...
  Future<void> handleToDeviceEvent(ToDeviceEvent event) async {
    if (event.type == EventTypes.SecretRequest) {
      ...
      final type = event.content.stringFor('name') ?? '';
      final secret = await getCached(type);
      if (secret == null) {
        Logs()
      ...
      final secret = event.content.stringFor('secret') ?? '';
      ...
      if (db != null) {
        ...
        if (keyId != null) {
          final ciphertext = client.accountData[request.type]!
              .content.tryMapFor('encrypted')?.tryMapFor(keyId)?.stringFor('ciphertext') ?? '';
  ...
  Set<String>? keyIdsFromType(String type) {
    ...
    if (data.content['encrypted'] is Map) {
      return (data.content.mapFor<String, dynamic>('encrypted') ?? {}).keys.toSet();
    }
    return null;
  }

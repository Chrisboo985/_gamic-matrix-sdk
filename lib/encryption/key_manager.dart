...
import 'package:matrix/extensions/map_extension.dart';
    ...
    encryption.ssss.setCacheCallback(megolmKey, (String secret) {
      ...
      for (final room in client.rooms) {
        ...
        if (lastEvent != null &&
          ...
          try {
            maybeAutoRequest(room.id, lastEvent.content.stringFor('session_id') ?? '',
                lastEvent.content.stringFor('sender_key') ?? '');
          } catch (_) {
            // dispose
          }
        }
    ...
    try {
      ...
      for (final roomEntry in keys.rooms.entries) {
        ...
        for (final sessionEntry in roomEntry.value.sessions.entries) {
          ...
          try {
            decrypted = json.decode(decryption.decrypt(sessionData.stringFor('ephemeral') ?? '',
                sessionData.stringFor('mac') ?? '', sessionData.stringFor('ciphertext') ?? ''));
          } catch (e, s) {
  ...
  Future<void> handleToDeviceEvent(ToDeviceEvent event) async {
    if (event.type == EventTypes.RoomKeyRequest) {
      ...
      if (event.content['action'] == 'request') {
        ...
        final room = client.getRoomById(event.content.tryMapFor('body')?['room_id']);
        if (room == null) {
          Logs().i('[KeyManager] Unknown room, ignoring');
          return; // unknown room
        }
        final sessionId = event.content.tryMapFor('body')?.stringFor('session_id') ?? '';
        final senderKey = event.content.tryMapFor('body')?.stringFor('sender_key') ?? '';
        ...
        final request = KeyManagerKeyShareRequest(
          requestId: event.content.stringFor('request_id') ?? '',
          devices: [device],
          room: room,
          sessionId: sessionId,
          senderKey: senderKey,
      ...
      event.content.listFor('forwarding_curve25519_key_chain')
      ...
      await setInboundGroupSession(
          ...
          forwarded: true,
          senderClaimedKeys: {
            'ed25519': event.content.stringFor('sender_claimed_ed25519_key') ?? '',
          });
      ...
      final String roomId = event.content.stringFor('room_id') ?? '';
      final String sessionId = event.content.stringFor('session_id') ?? '';

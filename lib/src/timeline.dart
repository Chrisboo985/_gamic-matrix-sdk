...
class Timeline {
      ...
      if (event.type == EventTypes.Encrypted &&
        ...
        try {
          room.client.encryption?.keyManager.maybeAutoRequest(
            room.id,
            event.content.stringFor('session_id'),
            event.content.stringFor('sender_key'),
  ...
  int _findEvent({String? event_id, String? unsigned_txid}) {
    ...
    for (i = 0; i < events.length; i++) {
      ...
      final txnid = events[i].unsigned?.stringFor('transaction_id');
      if (txnid != null) {
  ...
  void _removeEventFromSet(Set<Event> eventSet, Event event) {
    eventSet.removeWhere((e) =>
        ...
        (event.unsigned != null &&
            e.matchesEventOrTransactionId(event.unsigned?.stringFor('transaction_id'))));
  }

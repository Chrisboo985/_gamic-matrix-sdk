...
class Event extends MatrixEvent {
  ...
  String get messageType => type == EventTypes.Sticker
      ...
      : content.stringFor('msgtype') ?? MessageTypes.Text;
  ...
  String get text => content.stringFor('body') ?? '';

  /// Returns the formatted boy of this event if it has a formatted body.
  String get formattedText => content.stringFor('formatted_body') ?? '';
  ...
  Future<String?> sendAgain({String? txid}) async {
    ...
    return await room.sendEvent(
      content,
      txid: txid ?? unsigned?.stringFor('transaction_id') ?? eventId,
    );
  }
  ...
  Future<void> requestKey() async {
    ...
    await room.requestSessionKey(content.stringFor('session_id') ?? '', content.stringFor('sender_key') ?? '');
    return;
  }

  /// Gets the info map of file events, or a blank map if none present
  Map get infoMap => content.mapFor('info') ?? {};
  ...
  String get attachmentMimetype => infoMap['mimetype'] is String
      ...
      : content.mapFor('file')?.stringFor('mimetype') ?? '';
  ...
  Uri? get attachmentMxcUrl {
    final url = isAttachmentEncrypted ? content.mapFor('file')?.stringFor('url') : content.stringFor('url');
  ...
  String calcLocalizedBodyFallback(MatrixLocalizations i18n,
    ...
    if (hideEdit &&
      ...
      if (plaintextBody &&
          content.mapFor('m.new_content')?.stringFor('format') == 'org.matrix.custom.html') {
        htmlMessage = true;
  ...
  String? get relationshipType {
    ...
    if (content.mapFor('m.relates_to')?.containsKey('rel_type') == true) {
      if (content
    ...
    if (content.mapFor('m.relates_to')?.containsKey('m.in_reply_to') ?? true) {
